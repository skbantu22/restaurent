...
const BoxGift = async () => {
  ...
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/product/by-category?category=gifts&limit=4`
    );
    productData = response.data;
    console.log(productData);
  } catch (error) {
    console.error("Failed to fetch gift products:", error);
    productData = { success: false, data: [] }; // fallback
  }
    ...
    <div>
      <section className='lg:px-32 px-4 sm:py-4'>
        <div className='flex justify-between'>
          <h1 className='sm:text-4xl text-2xl font-semibold'>Gift Special Pricing</h1>
