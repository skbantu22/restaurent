// app/api/payment/sslcommerz/_ssl.js
...
export async function sslValidate(val_id) {
  ...
  if (!res.ok) {
    throw new Error(`SSL validate HTTP ${res.status}`);
  }
  return data;
}
...
export function pickSSLPaymentIndex(order, { tran_id }) {
  ...
  // Prefer match by tran_id (we stored tran_id in payment.paymentId)
  if (tran_id) {
    const idx = payments
      .map((p, i) => ({ p, i }))
  ...
  // fallback: last sslcommerz attempt
  const lastIdx = payments
    .map((p, i) => ({ p, i }))
    .filter(({ p }) => p?.method === "sslcommerz")
    .reverse()[0]?.i;

  return typeof lastIdx === "number" ? lastIdx : 0;
}
